namespace ex18_winControlApp
{
    ...
    {
        Random rand = new Random(); // 트리뷰 노드이름을 사용할 랜덤값
        public FrmMain()
        {
        ...
        void ChangeFont()
        {
            if (CboFonts.SelectedIndex < 0) // 아무것도 선택 안됨
                return;
            FontStyle style = FontStyle.Regular; // 일반 글자(볼드 X, 이탤릭X)로 초기화

            if (ChkBold.Checked)    // 굵게라는 체크 박스를 체크 하면
        ...
        {
            LsvDummy.Items.Add( // 리스트 뷰에 아이템 추가
                new ListViewItem(
        ...
        {
            PrgDummy.Value = TrbDummy.Value; // 트랙바 포인터를 옮기면 프로그레스 바 값도 같이 변경
        }
