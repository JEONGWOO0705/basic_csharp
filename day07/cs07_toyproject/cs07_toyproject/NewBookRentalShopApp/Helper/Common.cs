...
{
    public class Common  // internal 에서 public으로 바꾸기
    {
        // 정적으로 만드는 공통 연결 문자열 (static)
        ...
        // 회원 서택 팝업에서 대출화면으로 넘길 데이터 정적 프로퍼티
        ...
        public static string GetMd5Hash(MD5 md5Hash, string input)
        {
            ...
            StringBuilder builder = new StringBuilder(); // 문자열으 좀더 쉽게 쓰게 만들어주는 클래스
            for (int i = 0; i < data.Length; i++)
            {
                builder.Append(data[i].ToString("x2")); //16진수 문자로 각 글자를 변환
            }
