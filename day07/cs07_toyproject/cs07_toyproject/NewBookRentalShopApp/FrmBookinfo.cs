    ...
    public partial class FrmBookinfo : MetroForm
    {
        ...
        private void InitInputData()
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(Helper.Common.Connstring))
                {
                    ...
                    // SqlDataAdapder, DataSet = 한번에 데이터그리드뷰 등에 뿌릴때
        ...
        private void RefreshData()
        {
            using (SqlConnection conn = new SqlConnection(Helper.Common.Connstring))
            {
                ...
                DgvResult.Columns[0].HeaderText = "책 순번";
                DgvResult.Columns[1].HeaderText = "저자 명";
                DgvResult.Columns[2].HeaderText = "구분 코드";
                DgvResult.Columns[3].HeaderText = "구분 명";
        ...
        private void DgvResult_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1) // 아무것도 선택하지 않으면 -1
            {
                ...
                TxtAuthor.Text = selData.Cells[1].Value.ToString(); // 책 저자
                ...
                // "20000" 가격을 숫자형으로 형변환 해줌
                // 모든 타입에 .Parse(string) 메서드가 존재
